...
class SyntheticBurstPairDataset(SyntheticPairDataset):
    """ A synthetic generator of image pairs with burst applied to each image.
        Given a normal image dataset, it constructs pairs using random homographies & noise.
    """

    def __init__(self, dataset, scale='', distort='', crop_size=224, burst_size=5, shift_x=1, shift_y=0, mean=0,
                 sigma=255):
        super(SyntheticBurstPairDataset, self).__init__(dataset, scale, distort)
    ...
    def get_pair(self, i, output=('aflow')):
        img_a, img_b, meta = super(SyntheticBurstPairDataset, self).get_pair(i, output)
        img_a, img_b = generate_burst(img_a, img_b, crop_size=self.crop_size, burst_size=self.burst_size,
                                      shift_x=self.shift_x, shift_y=self.shift_y, mean=self.mean, sigma=self.variance)

        cropped_meta = {}
        for k in meta.keys():
            cropped_meta[k] = crop_center_array(meta[k], self.crop_size, self.crop_size)

        return img_a, img_b, cropped_meta
...
class TransformedBurstPairs(TransformedPairs):
    """ This function applies a series of random transformations to one original image
        to form a synthetic image pairs with burst and noise added with perfect ground-truth.
    """

    def __init__(self, dataset, trf='', crop_size=224, burst_size=5, shift_x=1, shift_y=0, mean=0,
                 sigma=255):
        super(TransformedBurstPairs, self).__init__(dataset, trf)
        self.crop_size = crop_size
        self.burst_size = burst_size
        self.shift_x = shift_x
        self.shift_y = shift_y
    ...
    def get_pair(self, i, output=('aflow')):
        img_a, img_b, meta = super(TransformedBurstPairs, self).get_pair(i, output)
        img_a, img_b = generate_burst(img_a, img_b, crop_size=self.crop_size, burst_size=self.burst_size,
                                      shift_x=self.shift_x, shift_y=self.shift_y, mean=self.mean, sigma=self.sigma)
        cropped_meta = {}
        for k in meta.keys():
            cropped_meta[k] = crop_center_array(meta[k], self.crop_size, self.crop_size)

        return img_a, img_b, meta
